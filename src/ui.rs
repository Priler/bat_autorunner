    ...
    match key {
        ...
        KeyCode::PageDown => {
            ...
            *scroll_offset = (*scroll_offset + visible_options).min(
                total_options.saturating_sub(visible_options)
            );
            Some(KeyAction::None)
        }
...
pub fn handle_selection(
    ...
    match &options[current_selection][..] {
            ...
            match run_powershell_command_with_output("Start-Process 'blockcheck.cmd'") {
                Ok(_) => {
                    execute!(
                        ...
                        Print("Blockcheck успешно запущен.")
                    )?;
                    std::process::exit(0);
                }
                Err(e) => {
                    execute!(
                        stdout,
                        cursor::MoveTo(0, message_row as u16 + 1),
                        Print(format!("Ошибка при запуске Blockcheck: {}", e))
                    )?;
                }
            }
        }
        selected_file => {
            ...
            service_manager.install_service(stdout, bat_file_path.to_str().unwrap(), message_row)?;
        }
    }
...
pub fn run_main_loop(
    ...
    let max_visible_options = std::cmp::min(
        15,
        (term_height as usize - start_row - 3)
    );
